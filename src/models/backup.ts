  ...
  private async refreshToken() {
    ...
    return new Promise(
      (resolve: (value: boolean) => void, reject: (reason: Error) => void) => {
        ...
        xhr.send(
          `client_id=${getCredentials().onedrive.client_id}&refresh_token=${UserSettings.items.oneDriveRefreshToken
          }&client_secret=${encodeURIComponent(
            getCredentials().onedrive.client_secret
          )}&grant_type=refresh_token&scope=https%3A%2F%2Fgraph.microsoft.com%2FFiles.ReadWrite${UserSettings.items.oneDriveBusiness !== true ? ".AppFolder" : ""
...
export class WebDAV implements BackupProvider {
  async upload(encryption: Encryption) {
    ...
    // Encrypt by default if user hasn't set yet - similar to other providers
    ...
    try {
      ...
      if (response.ok || response.status === 201 || response.status === 204) {
        if (config.retentionDays && config.retentionDays > 0) {
          // @ts-ignore
          await this.cleanupOldBackups(config, config.retentionDays);
        }
        return true;
      } else {
        console.error(`WebDAV upload failed with status: ${response.status}`);
        return false;
      }
    } catch (error) {
  ...
  private async cleanupOldBackups(config: { serverUrl: string; username: string; password: string }, retentionDays: number) {
    const { serverUrl, username, password } = config;
    ...
    try {
      ...
      for (const fileHref of filesToDelete) {
        ...
        console.log(`Deleted old backup: ${fileHref}`);
      }
