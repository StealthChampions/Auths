/**
 * Key Utilities Module | 密钥工具模块
 *
 * Core OTP code generation utilities.
 * Implements TOTP/HOTP algorithms with support for various hash functions.
 *
 * OTP 验证码生成核心工具。
 * 实现 TOTP/HOTP 算法，支持多种哈希函数。
 *
...
export class KeyUtilities {
  ...
  private static hex2str(hex: string) {
    ...
    for (let i = 0; i < hex.length; i += 2) {
      str += String.fromCharCode(this.hex2dec(hex.slice(i, i + 2)));
    }
    return str;
  }
  ...
  private static base32tohex(base32: string): string {
    ...
    switch (padding) {
      case 0:
        break;
      case 6:
        hex = hex.slice(0, hex.length - 8);
        break;
      case 4:
        hex = hex.slice(0, hex.length - 6);
        break;
      case 3:
  ...
  static generate(
    ...
    let otp =
      (this.hex2dec(hmac.slice(offset * 2, offset * 2 + 8)) & this.hex2dec("7fffffff")) +
      "";
