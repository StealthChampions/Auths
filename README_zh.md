...
- **安全存储**: 数据保留在本地。启用应用加密时，密钥使用 **AES-256-CBC + PBKDF2-SHA256**；带密码的备份导出使用 **AES-256-GCM + PBKDF2-SHA256**。
- **账户管理**: 支持通过文件夹对账户进行分类。**智能筛选**功能可根据当前网站自动显示相关验证码。
- **品牌图标**: 自动识别并匹配热门服务的品牌图标。
- **实时验证**: 精准的验证码生成与进度指示。
- **云端同步 (可选)**: 支持通过 **WebDAV** 备份与同步加密保险库。
...
我们视安全为生命线。
- **本地加密**: 应用加密使用 PBKDF2-SHA256 进行密钥派生，并使用 AES 加密。新的带密码备份导出使用 AES-GCM，旧版 AES-CBC 备份导出仍可导入。
